...
const sampleArtists = [
  // -------------- VISUAL --------------
  {
    ...
    about: "",
    artist_type: "visual",
    gallery: { title: "", description: "", entries: [] },
    id: nanoid(),
  },
  {
    first_name: "Leonardo da",
    last_name: "Vinci",
    alias: "",
    password: "",
    email: "",
...
const artist_Seeds = sampleArtists.map((artist) => {
  ...
  gallery.description =
    "This is an awesome gallery that brings out the best in all of the things that I care about.";
  ...
  async (newArtist) => {
    try {
      ...
      const response = await axios
        // .post("http://localhost:8000/createArtist/", newArtist)
        .post("http://localhost:8000/artists/", newArtist)
...
export const updateArtist = createAsyncThunk(
  "artist/updateArtist",
  async (id) => {
    const UPDATE_URL = `http://localhost:8000/updateArtist/${id}`;

    try {
      console.log("---------- UPDATE ARTIST ----------");
      const response = await axios.post(UPDATE_URL, id);
      console.log(response);
      return response;
    } catch (error) {
      return error.message;
...
export const deleteArtist = createAsyncThunk(
  "artist/deleteArtist",
  async (id) => {
    const DELETE_URL = `http://localhost:8000/deleteArtist/${id}`;
...
export const artistSlice = createSlice({
  ...
  reducers: {
    ...
    selectGalleryEntry: (state, action) => {
      ...
      const selected = artist.gallery.entries.filter(
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateArtist.fulfilled, (state, action) => {
        ...
        // const artists = state.bands
        const artists = state.bands.filter((band) => band._id === _id);
        console.log(artists);
        // console.log(action.payload)
